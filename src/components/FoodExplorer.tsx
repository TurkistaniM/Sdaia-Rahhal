...
import { getLocalizedFood } from '../data/translationsData';
import { Utensils, Search, Filter, X, MapPin, ChevronLeft, Sparkles, BookOpen } from 'lucide-react';
...
export const FoodExplorer: React.FC<FoodExplorerProps> = ({
  ...
  // Flatten and localize all foods
  const allFoods: FoodItem[] = useMemo(() => {
    return SAUDI_REGIONS.flatMap((region) => region.foods).map((food) =>
      getLocalizedFood(food, language)
    );
  }, [language]);
  ...
  const filteredFoods = useMemo(() => {
    return allFoods.filter((food) => {
      const matchesRegion =
        selectedRegionFilter === 'all' || food.regionId === selectedRegionFilter;
      const matchesSearch =
        food.arabicName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        food.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        food.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
        food.ingredients.some((ing) => ing.toLowerCase().includes(searchQuery.toLowerCase()));
      return matchesRegion && matchesSearch;
    });
  }, [allFoods, selectedRegionFilter, searchQuery]);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 animate-fadeIn">
      <div className="bg-[#FAF7F2] rounded-3xl w-full max-w-6xl h-[92vh] max-h-[92vh] border border-[#D4AF37] shadow-2xl flex flex-col overflow-hidden">
        ...
        {/* Foods Grid Scrollable Area */}
        <div className="flex-1 overflow-y-auto p-6">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-stone-500">
              {filteredFoods.length} {t.dishesCount}
            </span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredFoods.map((food) => {
              const regName = getRegionName(food.regionId, language);
              return (
                <div
                  key={food.id}
                    ...
                    <div className="pt-3 border-t border-stone-100 flex items-center justify-between">
                      <div className="flex flex-wrap gap-1">
                        {food.ingredients.slice(0, 2).map((ing, i) => (
                          <span
                            key={i}
                            className="px-2 py-0.5 rounded-md bg-[#FAF7F2] text-stone-600 text-[10px] font-medium border border-stone-200"
                          >
                            {ing}
                          </span>
                        ))}
      ...
      {/* Food Single Detail Popover Modal */}
      {selectedFoodDetail && (
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-[#D4AF37] shadow-2xl p-6 relative">
            ...
            <div className="flex items-center gap-2 mb-2">
              <span className="px-3 py-1 rounded-full bg-[#0F432A] text-[#FAF7F2] text-xs font-bold flex items-center gap-1">
                <MapPin className="w-3 h-3 text-[#D4AF37]" />
                {getRegionName(selectedFoodDetail.regionId, language)}
              </span>
            </div>
            ...
            <div className="space-y-4 bg-[#FAF7F2] p-5 rounded-2xl border border-[#D4AF37]/25 mb-6">
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {selectedFoodDetail.ingredients.map((ing, i) => (
                    <span
                      key={i}
                      className="px-2.5 py-1 rounded-lg bg-white text-stone-700 text-xs font-medium border border-stone-200"
                    >
                      {ing}
                    </span>
                  ))}
              ...
              <div>
                <h5 className="text-xs font-bold text-[#0F5132] mb-1">
                  {t.culturalContextTitle}:
                </h5>
                <p className="text-xs text-stone-600 leading-relaxed">
                  {selectedFoodDetail.culturalContext}
                </p>
              </div>

              {selectedFoodDetail.originStory && (
                <div>
                  <h5 className="text-xs font-bold text-[#0F5132] mb-1">
                    {t.originStoryTitle}:
                  </h5>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-stone-100">
              <button
                onClick={() => {
                  onSelectRegion(selectedFoodDetail.regionId);
                  onClose();
                }}
              ...
              <button
                onClick={() => {
                  const regName = getRegionName(selectedFoodDetail.regionId, language);
                  const prompt =
                    language === 'ar'
                      ? `حدثني بالتفصيل عن أكلة ${selectedFoodDetail.name} في ${regName} وطريقة إعدادها وقصتها التراثية.`
                      : language === 'fr'
                      ? `Parlez-moi en détail du plat ${selectedFoodDetail.name} de la région de ${regName}, de son histoire et de sa préparation.`
                      : language === 'es'
                      ? `Háblame en detalle sobre el plato ${selectedFoodDetail.name} de la región de ${regName}, su historia y preparación.`
                      : `Tell me in detail about the dish "${selectedFoodDetail.name}" from ${regName}, its traditional recipe, and cultural background.`;
