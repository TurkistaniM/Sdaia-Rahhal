      ...
      server.middlewares.use('/api/chat', async (req, res) => {
        ...
        req.on('end', async () => {
          try {
            ...
            const text = (data.prompt || data.message || '').trim();
            ...
            if (apiKey) {
              try {
                ...
                console.warn('Gemini API call failed, using offline cultural knowledge engine:', apiErr);
              }
            }

            // Fallback knowledge engine strictly grounded in site data
            const fallbackReply = generateKnowledgeFallback(text, language);
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
